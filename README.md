<!-- Core AI & ML -->
![Machine Learning](https://img.shields.io/badge/Machine%20Learning-FF6F00?style=for-the-badge)
![Deep Learning](https://img.shields.io/badge/Deep%20Learning-8E24AA?style=for-the-badge)
![ANN](https://img.shields.io/badge/ANN-6A1B9A?style=for-the-badge)
![CNN](https://img.shields.io/badge/CNN-1976D2?style=for-the-badge)
![RNN](https://img.shields.io/badge/RNN-0097A7?style=for-the-badge)

<!-- Generative AI -->
![Generative AI](https://img.shields.io/badge/Generative%20AI-000000?style=for-the-badge)
![LLMs](https://img.shields.io/badge/LLMs-4CAF50?style=for-the-badge)
![LangChain](https://img.shields.io/badge/LangChain-0A192F?style=for-the-badge)
![AI Assistants](https://img.shields.io/badge/AI%20Assistants-673AB7?style=for-the-badge)

<!-- Languages -->
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-336791?style=for-the-badge)
![C#](https://img.shields.io/badge/C%23-512BD4?style=for-the-badge&logo=dotnet&logoColor=white)

<!-- Frameworks & Tools -->
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=for-the-badge&logo=streamlit&logoColor=white)
![n8n](https://img.shields.io/badge/n8n-AE2C2C?style=for-the-badge)
![Git](https://img.shields.io/badge/Git-000000?style=for-the-badge&logo=git&logoColor=F05032)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github)

# 👋 Hi, I’m Mariam Ghareeb

### AI / ML & GenAI Engineer | Applied AI & Automation

I’m an AI-focused engineer passionate about building **practical, results-driven AI solutions**.
My work bridges **machine learning, generative AI, and intelligent automation**, with a strong focus on real-world impact, continuous learning, and collaboration.

---

## 🚀 What I Build
- 🤖 **AI & Machine Learning projects** (ML, DL, ANN, CNN, RNN)
- 🧠 **Generative AI applications** using LLMs & LangChain
- ⚙️ **Automation workflows** (n8n, intelligent pipelines)
- 📊 Data-driven tools that turn insights into action

---

## 🛠 Tech Stack
- **Languages:** Python, SQL, C#
- **AI / ML:** Machine Learning, Deep Learning, ANN, CNN, RNN
- **GenAI:** LangChain, LLM-based applications
- **Automation:** n8n
- **Tools:** Git, GitHub

---

## 📌 Featured Projects
> Selected projects demonstrating applied AI & automation

- 🔹 **GenAI Assistants & AI Apps**
- 🔹 **ML & DL Models (End-to-End)**
- 🔹 **Automation Tools & Workflows**
- 🔹 **Data & Analytics Solutions**


---

## 📚 Research & Continuous Learning
I actively study and document:
- AI & ML research papers
- GenAI advancements
- Books & learning resources that shape my thinking

➡️ Dedicated repository for **research notes, summaries & learning paths**

---

## 🤝 Let’s Collaborate
I’m open to:
- 💼 **AI / ML & GenAI roles**
- 🧩 **Freelance AI & automation projects**
- 🌍 **Collaborations & open-source contributions**

📫 Feel free to connect or reach out via LinkedIn.
---
https://www.linkedin.com/in/mariam-ghareeb-0a8517266/

⭐ **Always learning. Always building. Always improving.**
